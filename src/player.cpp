 ...
 * ------------------------------------------------------------------------- */
void Player::draw(float zoom, int vpx, int vpy)
{
    int px = (int)((_x/zoom - getCameraOrigX()) - (_txsize/zoom / 2));
    int py = (int)((_y/zoom - getCameraOrigY()) - (_tysize/zoom / 2));
    ...
    */
    ...
    if (zoom == 1) drawSprite(_frame, px+vpx, py+vpy);
    else drawSprite(_frame, px+vpx, py+vpy, zoom);
}
...
void Player::move(int d)
{
    ...
    switch (d)
    {
        ...
        {   if (!_frozen &&
            ...
            {
                if (!_holdingjump) { Audio::playSound(9); }
                _holdingjump = true;
                _yv = -_ymax;
...
void Player::tryFlipSwitch()
{
    ...
    // -- and not sitting reading a book (idle)                      --
    if (!_onground || (_action == SWITCHING && _actiontime > 0) ||
        _action == IDLE) return;
 ...
 *                                                                           *
 * Writes the player data to a map file; used by TED.                        *
 * ------------------------------------------------------------------------- */
void Player::writeMap(ofstream &file)
{
    file.write((char*)&_x, 4);
    file.write((char*)&_y, 4);
    file.write((char*)&_xdir, 4);
}
...
void Player::doPogoStick()
{
    ...
    {
        Audio::playSound(11);
        _yv -= _ymax * 2.25;                        // bounce
        _canjump = (int)(ts * _jumpheight * 2.25);  // pogo jumps higher
...
void Player::hitWall(int p, float hx, float hy)
{
    ...
    {
        case UP:
        _yv=0; _y=(int)(ny+(_ysize/2))+1; Audio::playSound(8); break;
 ...
 * void dropToGround()                                                       *
 *                                                                           *
 * Drops player to the ground, or the bottom of the map if no ground is      *
 * below him. It works like a modified doPhysics() method for dropping the   *
 * player until he collides with the map. Used for TED, called by setStart() *
 * ------------------------------------------------------------------------- */
void Player::dropToGround()
{
...
/* ------------------------------------------------------------------------- *
 * void setStart(int x, int y, int dir, bool putOnGround)                    *
 ...
 *                                                                           *
 * Sets the player's location and direction. Player will be dropped to the   *
 * ground if putOnGround is true.                                            *
 * ------------------------------------------------------------------------- */
void Player::setStart(int x, int y, int dir, bool putOnGround)
{
    _x = x; _y = y; _xdir = dir;
    if (putOnGround)
    {   _onground = false; _onslope = false; dropToGround(); }
    setFrameMoving();
}
...
void Player::setjumpcheat(bool j)     { _jumpcheat=j; }
...
void Player::setScuba(bool s)         { _scuba=s; }
...
bool  Player::jumpcheat()  { return _jumpcheat; }
