 ...
 *                                                                           *
 * Writes out all the shots for a saved game file.                           *
 * ------------------------------------------------------------------------- */
void Shot::writeSave(ofstream &file)
{
    writeInt(file, Shots.size());
    
    for (int i=0; i<Shots.size(); i++)
    {
        writeInt(file, Shots[i]->_type);
        writeFloat(file, Shots[i]->_x);
        writeFloat(file, Shots[i]->_y);
        writeFloat(file, Shots[i]->_xv);
        writeFloat(file, Shots[i]->_yv);
    }
}
 ...
 *                                                                           *
 * Reads in shots from a saved game file.                                    *
 * ------------------------------------------------------------------------- */
void Shot::readSave(ifstream &file)
{
    clearShots();
    int numshots = readInt(file);
    ...
    for (int i=0; i<numshots; i++)
    {
        ...
        float xv = readFloat(file);
        float yv = readFloat(file);

        Shot* s;
        switch (type)
        {   case BLASTER: s = new Blaster(x, y, UP); break;
            case ARROWUP: s = new ArrowUp(x, y); break;
            case ARROWDOWN: s = new ArrowDown(x, y); break;
            case ARROWLEFT: s = new ArrowLeft(x, y); break;
            case ARROWRIGHT: s = new ArrowRight(x, y); break;
        }
        s->_xv = xv; s->_yv = yv; addShot(s);
    }
}
 ...
 * ------------------------------------------------------------------------- */
Shot::Shot(int type, float x, float y, int dir, int sframe, int eframe,
    int delay, int xsize, int ysize, float velocity, int owner)
:_type(type), _x(x), _y(y), _frame(sframe), _sframe(sframe), _eframe(eframe),
 _delay(delay), _xsize(xsize), _ysize(ysize), _destroy(false), _owner(owner)
{
...
:Shot(BLASTER, x, y, dir, 705, 708, 1, 16, 16, 6, PLAYERSHOT)
...
{   Sprite::addSprite(_x, _y, _expsframe, _expeframe, _expdelay, _expxsize,
    ...
    if (inRange(240)) Audio::playSound(6);
}

/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */

ArrowUp::ArrowUp(float x, float y)
:Shot(ARROWUP, x, y, UP, 627, 628, 1, 8, 16, 10, ENEMYSHOT) {}

/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */

ArrowDown::ArrowDown(float x, float y)
:Shot(ARROWDOWN, x, y, DOWN, 629, 630, 1, 8, 16, 10, ENEMYSHOT) {}

/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */

ArrowLeft::ArrowLeft(float x, float y)
:Shot(ARROWLEFT, x, y, LEFT, 645, 646, 1, 16, 8, 10, ENEMYSHOT) {}

/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */

ArrowRight::ArrowRight(float x, float y)
:Shot(ARROWRIGHT, x, y, RIGHT, 647, 648, 1, 16, 8, 10, ENEMYSHOT) {}
