 ...
 *                                                                           *
 * Writes out all the sprites for a saved game file.                         *
 * ------------------------------------------------------------------------- */
void Sprite::writeSave(ofstream &file)
{
    writeInt(file, Sprites.size());
    
    for (int s=0; s<Sprites.size(); s++)
    {   file.write((char*)Sprites[s], sizeof(Sprite)); }
}
 ...
 *                                                                           *
 * Reads all the sprites from a saved game file.                             *
 * ------------------------------------------------------------------------- */
void Sprite::readSave(ifstream &file)
{
    clearSprites();
    int numsprites = readInt(file);
    int largestID = -1;
    
    for (int s=0; s<numsprites; s++)
    {   Sprite* spr = new Sprite();
        file.read((char*)spr, sizeof(Sprite));              // read sprite
        if (spr->_id > largestID) largestID = spr->_id;     // find largest id
        Sprites.push_back(spr);                             // store it
    }
    NextID = largestID + 1;
}
